...
--------+----------+-------------------+------------+----------
     ...
     15 | mi       | ni@gmail.com      | 8976554567 | 123
     17 | akshata  | akshata@gmail.com | 8623030243 | akshata
     18 | mahesh   | mahesh@gmail.com  | 9086759475 | mahesh
(6 rows)
...
tydb=# insert into parent values(default,'Sangita nale','sangita@gmail.com','01/23/1995','25','female','9087654345','Pimpri','','','mother');
INSERT 0 1
tydb=# select * from parent;
 pid |    pname     |       email       |    pdob    | page | gender | pmo_number | paddress | occupation |  waddress   | rel_with_child 
-----+--------------+-------------------+------------+------+--------+------------+----------+------------+-------------+----------------
   1 | Nikita Bhole | nikita@gmail.com  | 1981-07-12 |   39 | female | 9087655498 | Dhayari  | Job        | Aanadnagar  | mother
   4 | Sita Kale    | sita@gmail.com    | 1995-11-29 |   25 | female | 9876545678 | Pimpri   | clerk      | Sangvi      | mother
   2 | Ganesh More  | ganesh@gmail.com  | 1980-05-18 |   30 | male   | 9087667898 | Bavdhan  | Teacher    | Ganeshnagar | brother
   3 | Mahesh Marke | mahesh@gmail.com  | 1985-11-29 |   35 | male   | 8756434567 | Bavdhan  | Manager    | Hadapsar    | father
   5 | Sangita nale | sangita@gmail.com | 1995-01-23 |   25 | female | 9087654345 | Pimpri   |            |             | mother
(5 rows)
...
tydb=# insert into children values(default,'Riya','09/09/2010','10','O+','Pimpari','female','Profound','Mrs.Prakash Girme','9845321567',5);
INSERT 0 1
tydb=# select * from children;
 cid | cname |    cdob    | cage | blood_grp | caddress | cgender | disorder |    doctor_name    | dmo_number | pid 
-----+-------+------------+------+-----------+----------+---------+----------+-------------------+------------+-----
   1 | Kiran | 2003-12-08 |   17 | A+        | Dhayari  | male    | Mild     | Ms.Smita Rane     | 9087654678 |   1
   2 | Omkar | 2006-01-25 |   14 | AB+       | Pimpari  | male    | Severe   | Ms.Lila Mane      |  987656787 |   2
   3 | Gita  | 2013-07-09 |    6 | B+        | Bavdhan  | female  | Other    | Mrs.Eshwar Mane   | 4532345675 |   3
   4 | Disha | 2010-08-27 |   10 | O+        | Bavdhan  | female  | Profound | Mrs.Vijay Dhotre  | 8767867545 |   4
   5 | Riya  | 2010-09-09 |   10 | O+        | Pimpari  | female  | Profound | Mrs.Prakash Girme | 9845321567 |   5
(5 rows)
...
tydb=# create table admission(adid smallserial primary key not null,admission_type varchar(20) check(admission_type in('Full Day','Temporary','Residential')),admission_date date,ad_fees int ,ad_time varchar(20),pid int references parent on delete cascade on update cascade,cid int references children on delete cascade on update cascade);
CREATE TABLE
tydb=# \d admission;
...
tydb=# insert into admission values(default,'Temporary','07/10/2013','300','Based on daya','5','5');
INSERT 0 1
tydb=# select * from admission;
 adid | admission_type | admission_date | ad_fees |    ad_time    | pid | cid 
------+----------------+----------------+---------+---------------+-----+-----
    1 | Full Day       | 2007-12-07     |    3000 | 8am to 5pm    |   1 |   1
    2 | Full Day       | 2011-10-10     |    3000 | 8am to 5pm    |   2 |   2
    3 | Temporary      | 2018-01-09     |     300 | 2 days        |   3 |   3
    4 | Residential    | 2015-07-10     |     500 | Based on days |   4 |   4
    12| Temporary      | 2013-07-10     |     300 | Based on days |   5 |   5
(5 rows)

-----------------------------------------------------------------------------------
-------------Product-children(m-m)
------------------------------------------------------------------------------------

tydb=# create table pro_child(pno int references product on delete cascade on update cascade,cid int references children on delete cascade on update cascade,no_of_product_made int);
CREATE TABLE
tydb=# \d pro_child;
                   Table "public.pro_child"
       Column       |  Type   | Collation | Nullable | Default 
--------------------+---------+-----------+----------+---------
 pno                | integer |           |          | 
 cid                | integer |           |          | 
 no_of_product_made | integer |           |          | 
Foreign-key constraints:
    "pro_child_cid_fkey" FOREIGN KEY (cid) REFERENCES children(cid) ON UPDATE CASCADE ON DELETE CASCADE
    "pro_child_pno_fkey" FOREIGN KEY (pno) REFERENCES product(pno) ON UPDATE CASCADE ON DELETE CASCADE

tydb=# select * from pro_child;
 pno | cid | no_of_product_made 
-----+-----+--------------------
   1 |   1 |                  5
   1 |   2 |                 10
   2 |   1 |                  3
...
tydb=# create table feedback(name varchar(20),email varchar(25),message varchar(50));
CREATE TABLE
tydb=# \d feedback;
...
tydb=# insert into feedback values('Nikita Bhole','nikita@gmail.com','Very Good Website');
INSERT 0 1
tydb=# insert into feedback values('Sita Kale','sita@gmail.com','Please provide More information');
INSERT 0 1
tydb=# insert into feedback values('Ganesh More','ganesh@gmail.com','Nice');
INSERT 0 1
tydb=# insert into feedback values('Nikita Bhole','nikita@gmail.com','Manage more events');
INSERT 0 1
tydb=# select * from feedback;
     name     |      email       |             message             
--------------+------------------+---------------------------------
 Nikita Bhole | nikita@gmail.com | Very Good Website
 Sita Kale    | sita@gmail.com   | Please provide More information
 Ganesh More  | ganesh@gmail.com | Nice
 Nikita Bhole | nikita@gmail.com | Manage more events
(4 rows)
